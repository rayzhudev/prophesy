...
async function testCORS() {
  ...
  for (const origin of TEST_ORIGINS) {
    console.log(
      `\n${colors.bold}Testing with origin: ${origin || "No origin"}${
        colors.reset
      }`
    );
    console.log("=".repeat(50));

    // Only localhost and production frontend are allowed
    ...
    for (const endpoint of TEST_ENDPOINTS) {
        ...
        try {
          const preflightResponse = await fetch(`${baseUrl}${endpoint.url}`, {
            ...
            headers: {
              ...
              "Access-Control-Request-Headers": "content-type,x-trpc",
            },
          });
      ...
      try {
        ...
        const requestPassed = isAllowedOrigin
          ? response.status === 200 &&
